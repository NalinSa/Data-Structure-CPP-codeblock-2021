...
void get_num(int beginn,int endd, CP::map_bst<int,int> &bst){
    if(beginn==endd){
        bst.insert(std::make_pair(beginn,0));
        return;
    }
    if(beginn>endd){
        return;
    }
    int between = (beginn+endd)/2;
    bst.insert(std::make_pair(between,0));
    get_num(beginn,between-1,bst);
    get_num(between+1,endd,bst);
}
